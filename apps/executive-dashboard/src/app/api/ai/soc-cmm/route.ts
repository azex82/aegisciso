...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check user has permission for SOC-CMM assessments
    const userRole = (session.user as any).role;
    const allowedRoles = ['CISO', 'SOC_MANAGER', 'ADMIN', 'GRC_ANALYST'];
    if (!allowedRoles.includes(userRole)) {
      return NextResponse.json(
        { error: 'Insufficient permissions for SOC-CMM assessment' },
        { status: 403 }
      );
    }
    ...
    const response = await fetch(`${SOVEREIGN_AI_URL}/api/v1/compliance/soc-cmm`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error || 'SOC-CMM assessment failed' },
    ...
    console.error('SOC-CMM assessment error:', error);
    return NextResponse.json(
      { error: 'Failed to process SOC-CMM assessment' },
      { status: 500 }
    );
  }
}
